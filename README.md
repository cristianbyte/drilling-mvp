...
Use `updateHole(holeId, patch, supervisorName)` from `src/lib/firebase.js` to correct any hole by its Firebase ID. The `updatedAt` and `updatedBy` fields are written automatically — full audit trail.